...
var multiplayerMsg = "move server so yeah" //message when multiplayer is off
...
var serverInfo = [
  ...
  {url:"new-server.tommustbe12.repl.co"},
...
function valueToString(v, nf){ //for log
  ...
  if(typeof v === "function"){
    ...
    if(v.startsWith("MineKhan")){
      ...
      v = v.replace(/</g,"&lt;")
      v = v.replace(/>/g,"&gt;")
      if(v.startsWith("MineKhan: Message")){
        v = v.replace("Message","<b>Message</b>")
      }
    ...
    v = v.replace(/(added cape|removed cape|got cape called|changed their cape)/, "<span class='capeactivity'>$1</span>")
...
router.get("/panorama", (req,res) => {
  res.redirect("https://data.thingmaker.repl.co/images/panorama/8-12-2022.png")
})
...
router.get("/account", validate, async (request, response) => {
  ...
  try {
    await db.get("user:"+request.username)
      .then((result) => {
        if(!("profanityFilter" in result)) result.profanityFilter = true
...
router.post("/changeProfanityFilter", validate, async(req,res) => {
  if(!req.username) return res.status(401).json({message:"Unauthorized"})
  await getPostData(req)
  var user = await db.get("user:"+req.username)
...
router.post("/newMedia", async(req,res) => {
  ...
  var url = "https://server.thingmaker.repl.co/images/"+id+"."+type
  Log("Media url:",url)
  ...
  if(post.followers){
    for(var i=0; i<post.followers.length; i++){
      if(post.followers[i] !== req.username){
        notif(req.username+" edited post <a href='/website/post.html?id="+id+"'>"+post.title+"</a>", post.followers[i]).catch(e => {})
      }
    }
  }
    ...
    if(r.followers){
      for(var i=0; i<r.followers.length; i++){
        if(r.followers[i] !== req.username){
          notif(req.username+" commented at <a href='/website/post.html?id="+id+"#comment"+cid+"'>"+r.title+"</a>", r.followers[i]).catch(e => {})
        }
      }
    }
...
router.post("/commentUser/*", validate, async(req, res) => {
  if(!req.username) return res.json({message:"Sign in to comment"})
  ...
  await db.get("user:"+user).then(async r => {
    ...
    r.comments.push(commentData)
    if(req.username !== r.username) notif(req.username+" commented on <a href='/website/user.html?user="+user+"#comment"+cid+"'>your profile</a>", r)
    db.set("user:"+user, r).then(() => {
      ...
      Log("New comment at profile", "<a href='/website/user.html?user="+user+"#comment"+cid+"' target='_blank'>"+user+"</a>")
    })
  }).catch(e => {
    res.json(e)
    Log(e && e.message || e)
...
router.post("/deleteUserComment/*", validate, async(req,res) => {
  if(!req.username) return res.status(401).send("error")
  let user = req.url.split("/").pop()
  await getPostData(req)
...
router.post("/admin/giveCape/*", validate, async(req,res) => {
  if(!await isAdmin(req.username)) return res.json({message:"Unauthorized"})
  await getPostData(req)
  let to = req.url.split("/").pop()
...
minekhanWs.onrequest = function(request, connection, urlData) {
  ...
  connection.on('message', function(message) {
    ...
    if(data.type === "connect"){
      ...
      sendPlayers(JSON.stringify({
        type:"message",
        data: username+" is connecting. "+world.players.length+" players now."+(connection.isAdmin ? " "+username+" is admin." : ""),
        username: "Server",
        fromServer:true
      }))
      ...
      sendThisPlayer(JSON.stringify({
        type:"message",
        data: "Please read the multiplayer rules, read them <a href='https://www.thingmaker.repl.co/website/multiplayerRules.html' target='_blank'>here</a>. They can change at any time.",
        username: "Server",
        fromServer:true
      }))
      ...
      if(spyServers || world.spy) Log("MineKhan: Message from "+username+" in world "+world.name+": "+data.data)
    }else if(data.type === "setBlock" || data.type === "getSave" || data.type === "entityPos" || data.type === "entityPosAll" || data.type === "entityDelete" || data.type === "harmEffect" || data.type === "achievment" ||  data.type === "playSound" || data.type === "mySkin" || data.type === "setTags" || data.type === "entEvent" || data.type === "title" || data.type === "particles"){
      sendPlayers(message.utf8Data)
  ...
  async function getServerInfo(info){
    var data = await fetch("https://"+info.url+"/info").then(r => r.json()).catch(e => console.log(e))
